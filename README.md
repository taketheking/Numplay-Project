...
컴퓨터가 랜덤으로 생성한 3개 또는 그 이상의 숫자를 플레이어가 각 자리의 수를 순서에 상관없이 맞추면 되는 게임

예를 들어, 365 숫자가 랜덤으로 생성(보이지 않음)하면
사용자는 124(아웃), 123(1스트라이크 2볼), 354(2스트라이크 1볼)라는 숫자를 제시하면 순서에 상관없이 같은 숫자가 존재하기만 하면 스트라이크, 존재하지 않으면 볼로 된다.
